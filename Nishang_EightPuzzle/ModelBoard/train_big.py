...
def build_model(input_shape: Any) -> Any:
    ...
    # 增强的卷积神经网络结构
    x = layers.Conv2D(16, (3, 3), padding='same', activation='relu')(inputs)  # type:ignore
    x = layers.BatchNormalization()(x)  # type:ignore
    x = layers.MaxPooling2D((2, 2))(x)  # type:ignore
    x = layers.Dropout(0.2)(x)  # type:ignore

    x = layers.Conv2D(32, (3, 3), padding='same', activation='relu')(x)  # type:ignore
    x = layers.BatchNormalization()(x)  # type:ignore
    x = layers.MaxPooling2D((2, 2))(x)  # type:ignore
    x = layers.Dropout(0.3)(x)  # type:ignore

    x = layers.Conv2D(64, (3, 3), padding='same', activation='relu')(x)  # type:ignore
    x = layers.BatchNormalization()(x)  # type:ignore
    x = layers.MaxPooling2D((2, 2))(x)  # type:ignore
    x = layers.Dropout(0.4)(x)  # type:ignore

    x = layers.Conv2D(128, (3, 3), padding='same', activation='relu')(x)  # type:ignore
    x = layers.BatchNormalization()(x)  # type:ignore
    ...
    # 增强的全连接层
    x = layers.Dense(256, activation='relu')(x)  # type:ignore
    x = layers.BatchNormalization()(x)  # type:ignore
    x = layers.Dropout(0.5)(x)  # type:ignore
    
    x = layers.Dense(128, activation='relu')(x)  # type:ignore
    x = layers.BatchNormalization()(x)  # type:ignore
    x = layers.Dropout(0.4)(x)  # type:ignore
...
def main() -> None:
    try:
        ...
        # 直接保存h5模型
        model.save("digit_recognition_yuv_big.h5")  # type:ignore
        print("模型已保存为: digit_recognition_yuv_big.h5")
