...
int send_yuv422_to_pc(uint8_t *yuv_buf, uint32_t yuv_size,
	...
	// 5. 接收服务端响应
	LOG_I("Start receiving response\n");
	...
	// 接收完整响应(包含头部和内容)
	while ((n = read(sockfd, response + total, sizeof(response) - total - 1)) >
		   0) {
		total += n;
		if (total >= sizeof(response) - 1) {
			LOG_I("WARNING: Response buffer full\n");
			break;
		}
	}
	response[total] = '\0';
	...
	// 打印完整响应头
	LOG_I("Full response headers:\n");
	body_start = (uint8_t *)strstr(response, "\r\n\r\n");
	if (body_start) {
		...
		*body_start = '\r';	 // 恢复原始数据
		...
		LOG_I("WARNING: No header/body separator found\n");
		body_start = (uint8_t *)response;
	}
...
void cam_task(void *p) {
	...
	while (state) {
		...
		if (bflb_cam_get_frame_count(cam0) > 0) {
			...
			j++;
			send_yuv422_to_pc(pic, 240 * 240 * 2, "192.168.219.89", 9898);
			/*
