 ...
 *
 * 支持自定义接收函数，循环接收数据，处理超时、缓冲区溢出、连接关闭等情况，解析 HTTP 状态码与响应体。
 *
 ...
 *
 * 自动拼接标准 HTTP/1.1 请求头，包括 Host、Connection、User-Agent、Content-Type、Content-Length 及自定义头部。
 *
                               ...
                               char *buffer, int buffer_size)
{
    ...
    bool is_post_like = (strcmp(method, "POST") == 0 || strcmp(method, "PUT") == 0);
    ...
    ret = snprintf(buffer + written_len, buffer_size - written_len, "Host: %s\r\n", server);
    if (ret < 0 || ret >= (buffer_size - written_len)) {
        return REQ_ERR_BAD_REQUEST;
    }
    written_len += ret;

    ret = snprintf(buffer + written_len, buffer_size - written_len, "Connection: close\r\n");
    if (ret < 0 || ret >= (buffer_size - written_len)) {
        return REQ_ERR_BAD_REQUEST;
    }
    written_len += ret;

    ret = snprintf(buffer + written_len, buffer_size - written_len, "User-Agent: %s\r\n", DEFAULT_USER_AGENT);
    if (ret < 0 || ret >= (buffer_size - written_len)) {
        return REQ_ERR_BAD_REQUEST;
    }
    written_len += ret;
    ...
    if (is_post_like) {
        ret = snprintf(buffer + written_len, buffer_size - written_len, "Content-Type: %s\r\n", content_type);
        if (ret < 0 || ret >= (buffer_size - written_len)) {
            return REQ_ERR_BAD_REQUEST;
        }
        written_len += ret;

        ret = snprintf(buffer + written_len, buffer_size - written_len, "Content-Length: %d\r\n", body_len);
        if (ret < 0 || ret >= (buffer_size - written_len)) {
            return REQ_ERR_BAD_REQUEST;
        }
        written_len += ret;
...
static int _send_data_internal(int32_t fd,
                               int (*send_func)(int32_t fd, const uint8_t *data, size_t len, int flags),
 ...
 * @brief HTTP 请求主流程
 *
 * 封装 HTTP 请求的完整流程，包括连接、请求头构建与发送、请求体发送、响应接收与解析、资源清理等。
 *
...
int _http_request_internal(const http_request_config_t *config, const http_proto_ops_t *ops)
{
    ...
    if (!config || !ops || !config->server || !config->path || !config->method ||
        !config->response_buf || config->response_buf_len <= 0 || !config->actual_resp_len_ptr ||
    ...
    *config->actual_resp_len_ptr = 0;
    ...
    int connect_status = ops->connect(config->server, config->port, config->timeout_ms, &fd);
    ...
    header_len = _build_http_request_header(
        config->method, config->path, config->server,
        config->content_type, config->body_len, config->custom_headers,
        request_header_buf, MAX_HEADER_BUF_SIZE);
    ...
    ret_code = _send_data_internal(fd, ops->send, (const uint8_t *)request_header_buf, header_len, start_us, config->timeout_ms);
    ...
    if (config->request_body && config->body_len > 0) {
        ret_code = _send_data_internal(fd, ops->send, (const uint8_t *)config->request_body, config->body_len, start_us, config->timeout_ms);
        if (ret_code != REQ_OK) {
            goto cleanup_internal;
        }
    }

    ret_code = _process_response(ops->recv, fd,
                                 config->response_buf, config->response_buf_len,
                                 config->actual_resp_len_ptr,
                                 start_us, config->timeout_ms);
