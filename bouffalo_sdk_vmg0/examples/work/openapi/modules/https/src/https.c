...
*/
    // 解析 ISRG Root X1 (Let's Encrypt)
    ret = mbedtls_x509_crt_parse(&bl_hsbuf->cacert, (const unsigned char *)ISRG_ROOT_X1_PEM,
                                 sizeof(ISRG_ROOT_X1_PEM)); // 使用 sizeof 获取 PEM 字符串长度 (包含末尾的 '\0')
    if (ret < 0) {
        printf("Failed to parse ISRG_ROOT_X1_PEM: -0x%x\n", -ret);
        // 根据需要添加错误处理
    } else {
        printf("Parsed ISRG_ROOT_X1_PEM successfully.\n");
    }

    // 解析 DigiCert Global Root G2
    ret = mbedtls_x509_crt_parse(&bl_hsbuf->cacert,
                                 (const unsigned char *)DIGICERT_GLOBAL_ROOT_G2_PEM,
                                 sizeof(DIGICERT_GLOBAL_ROOT_G2_PEM));
    if (ret < 0) {
        printf("Failed to parse DIGICERT_GLOBAL_ROOT_G2_PEM: -0x%x\n", -ret);
        // 根据需要添加错误处理
    } else {
        printf("Parsed DIGICERT_GLOBAL_ROOT_G2_PEM successfully.\n");
    }

    // 解析 Starfield Services Root Certificate Authority - G2
    ret = mbedtls_x509_crt_parse(&bl_hsbuf->cacert,
                                 (const unsigned char *)STARFIELD_SERVICES_ROOT_G2_PEM,
                                 sizeof(STARFIELD_SERVICES_ROOT_G2_PEM));
    if (ret < 0) {
        printf("Failed to parse STARFIELD_SERVICES_ROOT_G2_PEM: -0x%x\n", -ret);
        // 根据需要添加错误处理
    } else {
        printf("Parsed STARFIELD_SERVICES_ROOT_G2_PEM successfully.\n");
    }

    // 解析 DigiCert Global Root CA
    ret = mbedtls_x509_crt_parse(&bl_hsbuf->cacert,
                                 (const unsigned char *)DIGICERT_GLOBAL_ROOT_CA_PEM,
                                 sizeof(DIGICERT_GLOBAL_ROOT_CA_PEM));
    if (ret < 0) {
        printf("Failed to parse DIGICERT_GLOBAL_ROOT_CA_PEM: -0x%x\n", -ret);
        // 根据需要添加错误处理
    } else {
        printf("Parsed DIGICERT_GLOBAL_ROOT_CA_PEM successfully.\n");
    }
        ...
        if (FD_ISSET(tcp_fd, &wset) != 0) {
            ...
            if (pssl->state != MBEDTLS_SSL_HANDSHAKE_OVER) {
                ...
                if ((0 != ret) && (MBEDTLS_ERR_SSL_WANT_READ != ret)) {
                    printf("[E: blTcpSslState] mbedtls_ssl_handshake_step failed with mbedtls error: -0x%X\n", -ret);
                    errcode = BL_TCP_CONNECT_ERR;
                }
            } else {
