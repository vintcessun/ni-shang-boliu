...
def lasr_example():
    ...
    # 设置是否需要话者分离，默认False，需要need_analysis_info设置为True才生效。
    asrc_request.set_diarization(False)
    # 设置声道信息, 一般都是单声道，默认为MONO，需要need_analysis_info设置为True才生效
    asrc_request.set_channel('MONO')
    # 设置是否返回感情信息, 默认False，需要need_analysis_info设置为True才生效。
    asrc_request.set_emotion(False)
    # 设置是否需要返回语速信息，默认False，需要need_analysis_info设置为True才生效。
    asrc_request.set_speed(False)
