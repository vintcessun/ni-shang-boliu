...
#define TASK_PRIORITY   (tskIDLE_PRIORITY + 2) // 任务优先级 (比 IDLE 高即可)
...
static void auto_connect_task(void *pvParameters)
{
    ...
    if (xSemaphoreTake(wifi_ready_sem, portMAX_DELAY) == pdTRUE) {
        LOG_I("[%s] WiFi ready signal received. Preparing to connect to [%s]...\r\n", TASK_NAME, g_ssid);
        ...
        // --- 修改处：尝试调用 wifi_sta_connect() 而不是 wifi_mgmr_sta_connect() ---
        ...
        if (result == 0) {
            LOG_I("[%s] Connection request (wifi_sta_connect) sent successfully.\r\n", TASK_NAME);
            // 看看这次是否能像手动输入命令一样连接成功 (可能也需要第二次尝试，由 autoconnect 处理)
        } else {
            LOG_E("[%s] Failed to send connection request (wifi_sta_connect), error code: %d\r\n", TASK_NAME, result);
        }
    } else {
...
void auto_connect_init(const char *ssid, const char *key) // 接收 SSID 和 Key
{
    ...
    // 创建任务
    xTaskCreate(auto_connect_task, TASK_NAME, TASK_STACK_SIZE, NULL, TASK_PRIORITY, NULL);
...
void auto_connect_signal_wifi_ready(void)
{
    if (wifi_ready_sem != NULL) {
        ...
        // 如果需要，进行上下文切换 (通常在 FreeRTOS 的 portYIELD_FROM_ISR 中处理)
