...
#define TEST_ADC_CHANNELS                                           \
	(TEST_ADC_CHANNEL_0 + TEST_ADC_CHANNEL_1 + TEST_ADC_CHANNEL_2 + \
	 TEST_ADC_CHANNEL_3 + TEST_ADC_CHANNEL_4 + TEST_ADC_CHANNEL_5 + \
	 TEST_ADC_CHANNEL_6 + TEST_ADC_CHANNEL_7 + TEST_ADC_CHANNEL_8 + \
	 TEST_ADC_CHANNEL_9 + TEST_ADC_CHANNEL_10)
...
ATTR_NOCACHE_NOINIT_RAM_SECTION uint32_t
	raw_data[TEST_ADC_CHANNELS * TEST_COUNT];
...
int main(void) {
	...
	while (1) {
		...
		if (last != result.millivolt) {
			last = result.millivolt;
			printf("raw data:%08x\r\n", raw_data[0]);
			printf("pos chan %d,%d mv \r\n", result.pos_chan, result.millivolt);
		}
	}
