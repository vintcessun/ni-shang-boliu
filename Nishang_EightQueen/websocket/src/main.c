...
bool websocket_connect(websocket_client_t *client) {
	...
	// 构建握手请求
	// LOG_I("构建握手请求\n");
	char *handshake_request = (char *)pvPortMalloc(strlen(client->path) + 600);
...
int websocket_send(websocket_client_t *client, ws_opcode_t opcode,
	...
	// 应用掩码并发送数据（非阻塞方式）
	uint8_t *masked_data = (uint8_t *)pvPortMalloc(len * sizeof(uint8_t));
	if (!masked_data) {
